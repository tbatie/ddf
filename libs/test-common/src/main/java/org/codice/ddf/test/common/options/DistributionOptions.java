    ...
    if (SystemUtils.IS_OS_WINDOWS) {
      ...
      distroOption =
          new KarafDistributionKitConfigurationOption(
              ...
              .executable("bin/ddf")
              .filesToMakeExecutable(
                  "bin/karaf", "bin/setenv", "bin/start", "bin/client", "bin/shell", "bin/stop", "bin/solr", "solr/bin/solr")
              .unpackDirectory(UNPACK_DIRECTORY)
              .useDeployFolder(false);
    }
    return new DefaultCompositeOption(distroOption, mavenRepos(), editConfigurationFilePut(SYSTEM_PROPERTIES_FILE_PATH, "start.solr", Boolean.toString(startSolr)));
  }
